    ...
    def check(self) -> HealthCheckResult:
        ...
        try:
            ...
            if free_gb < self.min_free_gb:
                status, message = HealthStatus.UNHEALTHY, f"Low disk space: {free_gb:.1f}GB free"
            elif free_gb < self.min_free_gb * 2:
                status, message = HealthStatus.DEGRADED, f"Disk space low: {free_gb:.1f}GB free"
            else:
                status, message = HealthStatus.HEALTHY, f"Disk space OK: {free_gb:.1f}GB free"
    ...
    def check(self) -> HealthCheckResult:
        ...
        try:
            ...
            if used_percent > self.max_usage_percent:
                status, message = HealthStatus.UNHEALTHY, f"High memory usage: {used_percent:.1f}%"
            elif used_percent > self.max_usage_percent * 0.8:
                status, message = HealthStatus.DEGRADED, f"Memory usage elevated: {used_percent:.1f}%"
            else:
                status, message = HealthStatus.HEALTHY, f"Memory usage OK: {used_percent:.1f}%"
            return HealthCheckResult(
                name=self.name, status=status, message=message,
